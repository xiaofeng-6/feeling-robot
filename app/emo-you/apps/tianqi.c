...
void tianqi(void)
{
    ...
    lv_label_set_text(max_temp_label, "28度");
    lv_obj_set_style_text_color(max_temp_label, lv_color_hex(0xff4444), 0);
    lv_obj_set_style_text_font(max_temp_label, &lv_font_biaozhunheiti_16, 0);
    lv_obj_align(max_temp_label, LV_ALIGN_CENTER, 0, 0);
    
    // 添加标题标签
    lv_obj_t * max_temp_title = lv_label_create(scr);
    lv_label_set_text(max_temp_title, "最高温度");
    lv_obj_set_style_text_color(max_temp_title, lv_color_hex(0x666666), 0);
    lv_obj_set_style_text_font(max_temp_title, &lv_font_biaozhunheiti_16, 0);
    lv_obj_set_pos(max_temp_title, 35, 95);
    ...
    for(int i = 0; i < 6; i++) {
        ...
        lv_obj_set_style_text_font(time_label, &lv_font_biaozhunheiti_16, 0);
...
void tianqi_with_data(WeatherData *weather_data)
{
    if (weather_data == NULL) {
        osal_printk("weather_data为空，使用默认天气显示\r\n");
        tianqi();  // 调用原始函数
        return;
    }

    osal_printk("tianqi_with_data: 最高温度=%d, 最低温度=%d\r\n", 
                weather_data->max_temp_current, weather_data->min_temp_current);
    ...
    lv_obj_clear_flag(max_temp_arc, LV_OBJ_FLAG_CLICKABLE);
    lv_arc_set_range(max_temp_arc, 0, 40);
    lv_arc_set_value(max_temp_arc, weather_data->max_temp_current);  // 使用动态数据
    lv_obj_center(max_temp_arc);
    
    // 添加中心温度标签
    lv_obj_t * max_temp_label = lv_label_create(max_temp_scale);
    char max_temp_text[16];
    snprintf(max_temp_text, sizeof(max_temp_text), "%d度", weather_data->max_temp_current);
    lv_label_set_text(max_temp_label, max_temp_text);
    lv_obj_set_style_text_color(max_temp_label, lv_color_hex(0xff4444), 0);
    lv_obj_set_style_text_font(max_temp_label, &lv_font_biaozhunheiti_16, 0);
    lv_obj_align(max_temp_label, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_obj_clear_flag(min_temp_arc, LV_OBJ_FLAG_CLICKABLE);
    lv_arc_set_range(min_temp_arc, -10, 30);
    lv_arc_set_value(min_temp_arc, weather_data->min_temp_current);  // 使用动态数据
    lv_obj_center(min_temp_arc);
    
    // 添加中心温度标签
    lv_obj_t * min_temp_label = lv_label_create(min_temp_scale);
    char min_temp_text[16];
    snprintf(min_temp_text, sizeof(min_temp_text), "%d度", weather_data->min_temp_current);
    lv_label_set_text(min_temp_label, min_temp_text);
    lv_obj_set_style_text_color(min_temp_label, lv_color_hex(0x4488ff), 0);
    lv_obj_set_style_text_font(min_temp_label, &lv_font_biaozhunheiti_16, 0);
    lv_obj_align(min_temp_label, LV_ALIGN_CENTER, 0, 0);
    ...
    // 设置最高温度数据（使用动态数据）
    for(int i = 0; i < 6; i++) {
        lv_chart_set_next_value(temp_chart, max_temp_series, weather_data->max_temp_periods[i]);
    }
    
    // 设置最低温度数据（使用动态数据）
    for(int i = 0; i < 6; i++) {
        lv_chart_set_next_value(temp_chart, min_temp_series, weather_data->min_temp_periods[i]);
    }
