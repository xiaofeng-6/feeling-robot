...
void surprised(void)
{
    ...
    /* ---------------- 画第一条红色弧线 ---------------- */
    {
        /* 使用三次贝塞尔曲线，起点(100,100)，终点(100,200)，控制点(140,80)和(200,120) */
        const int POINT_CNT = 21; /* 取21个采样点 */
        static lv_point_precise_t red_arc1_points[21];

        const float x0 = 100.0f, y0 = 100.0f;  /* 起点 */
        const float x3 = 100.0f, y3 = 200.0f;  /* 终点 */
        const float x1 = 140.0f, y1 = 80.0f;   /* 第一个控制点 */
        const float x2 = 200.0f, y2 = 120.0f;  /* 第二个控制点 */
    ...
    /* ---------------- 画第二条红色弧线 ---------------- */
    {
        /* 使用三次贝塞尔曲线，起点(100,100)，终点(100,200)，控制点(60,80)和(20,120) */
        const int POINT_CNT = 21; /* 取21个采样点 */
        static lv_point_precise_t red_arc2_points[21];

        const float x0 = 100.0f, y0 = 100.0f;  /* 起点 */
        const float x3 = 100.0f, y3 = 200.0f;  /* 终点 */
        const float x1 = 60.0f, y1 = 80.0f;    /* 第一个控制点 */
        const float x2 = 0.0f, y2 = 120.0f;    /* 第二个控制点 */
